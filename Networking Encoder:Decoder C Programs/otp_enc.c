// otp_enc.c:: CS_344_400  -- Program 4 - one-time-pad encryption using networks
...
// We send a plaintext and key file to server, and recieve back an encrypted
// file.  This gets output to stdout.
...
int main(int argc, char *argv[])
{
    ...
    int new_socket;
    long plaintext_size, key_file_size;
...
//////    printf("Connected to main server port: %i\n",port_main_running);
    ...
    plaintext_size = writeFile(new_socket,plaintext);
 //////   printf("Plaintext file size: %li\n",plaintext_size);
    //WRITE KEY FILE
    key_file_size = writeFile(new_socket,key_file);
 //////   printf("Key file size: %li\n",key_file_size);
    
    if(key_file_size < plaintext_size)
    {
...
//////            printf("Attempt to connect to port: %i\n",newPort);
            ...
            {
                strcpy(connection_status, "11Bad");  //Send string represents bad connection
...
long writeFile(int new_conn_file_d,const char* fileName)
{
    ...
    fptr = fopen(fileName,"r");  //Open current file
    if(fptr == NULL)
    {
    ...
    do
    {
        found_char=0;  //Initally set found_char and new line found to 0
        new_line_found=0;  
...
// server (opt_enc_d) is the one we are trying to connect to.  If not, we will
// get message (No), else good connection.
//================
void checkCorrectServer(int localFileD)
{
    char validate_connection[20]= "Conn to opt_enc_d";
...
//Description: Get encoded output from the server, and direct to stdout.
...
//================
void readAndOutputEnc(int new_socket)
{
     ...
     while(reading==1)
        {
            ...
            {  //Invlaid character found by server
